...
class URL(Base):
    ...
    is_active = Column(Integer, default=True) # Using Integer/Boolean depending on DB schema, assuming Boolean mapped
...
async def get_analytics(short_code: str, db: AsyncSession = Depends(get_db)):
    ...
    # Total clicks
    result = await db.execute(text("SELECT COUNT(*) as total FROM analytics WHERE url_id = :url_id"), {"url_id": url_id})
    total_clicks = result.scalar()

    # Unique IPs
    result = await db.execute(text("SELECT COUNT(DISTINCT ip_address) as unique_ips FROM analytics WHERE url_id = :url_id"), {"url_id": url_id})
    unique_ips = result.scalar()

    # Clicks today
    today_start = now.replace(hour=0, minute=0, second=0, microsecond=0)
    result = await db.execute(
        text("SELECT COUNT(*) as clicks_today FROM analytics WHERE url_id = :url_id AND clicked_at >= :today_start"),
    ...
    result = await db.execute(
        text("SELECT COUNT(*) as clicks_week FROM analytics WHERE url_id = :url_id AND clicked_at >= :week_start"),
    ...
    result = await db.execute(
        text("SELECT COUNT(*) as clicks_month FROM analytics WHERE url_id = :url_id AND clicked_at >= :month_start"),
    ...
    top_referrers = [{"referrer": row.referrer, "count": row.count} for row in result.fetchall()]
    ...
    clicks_by_date = [{"date": str(row.date), "clicks": row.clicks} for row in result.fetchall()]
...
async def get_click_records(
    ...
    # Get click records
    query = select(Analytics).where(Analytics.url_id == url_data.id).order_by(Analytics.clicked_at.desc()).offset(skip).limit(limit)
    result = await db.execute(query)
...
async def get_user_analytics_summary(user_id: int, db: AsyncSession = Depends(get_db)):
    ...
    # Total URLs
    result = await db.execute(text("SELECT COUNT(*) as total_urls FROM urls WHERE user_id = :user_id"), {"user_id": user_id})
    ...
    # Active URLs
    result = await db.execute(text("SELECT COUNT(*) as active_urls FROM urls WHERE user_id = :user_id AND is_active = TRUE"), {"user_id": user_id})
...
async def delete_analytics(short_code: str, db: AsyncSession = Depends(get_db)):
    ...
    await db.execute(text("DELETE FROM analytics WHERE url_id = :url_id"), {"url_id": url_data.id})
    await db.commit()
